...
## Phase 3: 전문 검색 및 의미 기반 개념 추출

**BM25 전문 검색**: 순수 JavaScript로 구현한 BM25 알고리즘으로 단순 부분문자열 매칭을 넘어 한국어 문서의 의미 있는 검색을 제공합니다. 전체 문서를 토큰화하여 역색인을 구축하고, 서버의 `/api/search` 엔드포인트와 `/api/ask`의 근거 문서 선정에 모두 사용됩니다.

**Ollama 기반 임베딩 개념 클러스터링**: `--ollama-embeddings` 플래그를 사용하면 문서 임베딩을 바탕으로 개념을 자동 추출합니다. 각 문서를 벡터로 변환한 후 k-means 클러스터링을 거쳐 의미상 유사한 문서들을 그룹화하고, 각 그룹의 지배적 키워드로 개념 라벨을 자동 생성합니다. Ollama가 미설치되거나 임베딩 실패 시에는 기존의 TF-IDF 키워드 빈도 기반 추출로 자동 폴백되어, 항상 완전한 위키를 생성합니다.

**AI 생성 질문 연결**: Ollama로 생성한 검색 질문들을 Search Contract에 직접 연결하여, AI와 사람이 예상하는 검색 시나리오를 문서화합니다.
