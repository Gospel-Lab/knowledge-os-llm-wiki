...
**오프라인 우선 (Offline-First)**: 생성된 3D 그래프의 모든 브라우저 라이브러리(D3, Three.js, 폰트)를 로컬에 벤더링하여 CDN/Google Fonts 의존성을 제거했습니다. 인터넷이 끊겨도 그래프 렌더링이 완전히 작동합니다.
...
**Ollama 캐싱 및 병렬화**: Ollama 질의응답이 여러 문서에서 동일한 쿼리를 캐싱하여 중복 계산을 줄입니다. 또한 Ollama 호출을 병렬로 처리하여 대규모 문서 처리 속도를 향상시킵니다.
