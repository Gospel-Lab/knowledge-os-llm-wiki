...
// 형태소 분석은 하지 않는다 — 공백/구두점 기준 토큰화 + 불용어 제거 + 빈도 기반에,
// 한국어 어절 끝의 조사를 규칙 기반으로 스트리핑하고 서술어(동사/형용사) 어미로
// 끝나는 토큰을 통째로 걸러내는 휴리스틱을 더한 것이다. 완벽하진 않지만,
// "이 문서들은 비슷한 단어를 많이 공유한다"는 자동 링크 신호로는 충분하다.
...
const PREDICATE_ENDING_RE =
  /(니다|니까|십시오|하세요|세요|어요|아요|에요|예요|았다|었다|였다|한다|된다|하다|되다|이다|하며|되며|하고|되고|하여|되어|해서|돼서|하면|되면|하지|되지|겠다|는다)$/;
...
export function tokenize(text) {
  const raw = text.normalize('NFC').toLowerCase().match(/[\p{L}\p{N}]{2,}/gu) || [];
  const out = [];
